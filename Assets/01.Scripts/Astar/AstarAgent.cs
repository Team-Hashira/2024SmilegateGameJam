    ...
    public class AstarAgent : MonoBehaviour, IUnitComponent
    {
        ...
        private bool CalcRoute()
        {
            ...
            while (_openList.Count > 0)
            {
                ...
                // 현재 노드가 목적지에 가장 가까운지 확인
                float curDist = CaclH(n.pos);
            ...
            if(!result)
            {
                Debug.LogWarning("나는 경로를 찾지 못하다!!!");
            }
        ...
        private void FindOpenList(Node currentNode)
        {
            for (int i = -1; i <= 1; i++)
            {
                for (int j = -1; j <= 1; j++)
                {
                    ...
                    if (_cornerCheck &&
                        (!Map.CanMove(new Vector3Int(next.x, currentNode.pos.y, 0), _owner)
                        || !Map.CanMove(new Vector3Int(currentNode.pos.x, next.y, 0), _owner)
                        ))
                    {
                        continue;
                    }

                    if (Map.CanMove(next, _owner))
                    {
